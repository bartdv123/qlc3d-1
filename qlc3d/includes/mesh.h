...
public:
  ...
  inline idx getNode(const idx e, const idx n) const
  { // returns node n of element e
#ifdef DEBUG
  ...
  vector<set<idx>> gen_p_to_elem_return(vector<set<idx>> &p_to_elem) const;

  void findNodesConnectedToNode(const Mesh &mesh, idx nodeIndex, std::set<idx> &connectedNodes, vector<set<idx>> &map) const;
};
