...
Geometry::Geometry() : regularGrid(nullptr)
{
  npLC = 0;
  t = Mesh::tetMesh();
  e = Mesh::triangleMesh();
...
void Geometry::calculateNodeNormals()
{
  ...
  { // add neighbouring surface normals
    int m = e->getMaterialNumber(i);
    if (MATNUM_TO_FIXLC_NUMBER(m))
    { // IF FIXLC SURFACE
...
void Geometry::ReorderDielectricNodes()
{
  ...
  {
    if (tets.getMaterialNumber(i) >= MAT_DIELECTRIC1)
    { // if material nuber > LC number
      DE_exist = true;
      break;
    }
  }
  setnpLC(getnp());
  if (!DE_exist)
  { // if no dielectric materials -> no need to reorder = exit
                                ...
                                const int &norm)
{
  ...
  for (F0 = face0.begin(), fc = 0; F0 != face0.end(); ++F0, ++fc)
  { // LOOP OVER FACE 0
    bool found = false;
    ...
    for (F1 = face1.begin(), bc = 0; F1 != face1.end(); ++F1, ++bc)
    {
      ...
      if (tdist < dist)
      {               // debug info only, keep track of nearest found node
...
#define TOP (getAbsZDist(n, zmax) <= eps)
      ...
      !gettop_bottom_is_periodic())
  {
    ...
    for (size_t i = 0; i < nodes.size(); i++)
    {
      ...
      if (FRONT)
      { // check if node i is on front surface
        front.push_back(n);
      }
      else if (BACK)
      { // check if node i is on back surface
        back.push_back(n);
      }
      else
      { // ERROR
...
void Geometry::initialisePeriodicity()
{
  ...
  for (idx i = 0; i < e->getnElements(); i++)
  {
    if (e->getMaterialNumber(i) == MAT_PERIODIC)
    { // if surface is periodic
...
)
{
  ...
  for (idx i = 0; i < t->getnElements(); i++)
  {
    ...
    if (found)
    { // If coord is in tet i
      if (requireLCEelement)
      { // WANT LC
        if (t->getMaterialNumber(i) <= MAT_DOMAIN7)
        { // IF LC
          ind = i;
          return true;
        }
    ...
    const bool &requireLCElement)
{
  ...
  {
    if (!requireLCElement)
    { // if not worried about whether LC or DE element
      return currentTetIndex;
    }
    else
    { // LC element is required
      if (this->t->getMaterialNumber(currentTetIndex) <= MAT_DOMAIN7)
      { // if LC element
        return currentTetIndex;
      }
    }
  ...
  if ((idx)tetHistory.size() > RECURSION_LIMIT)
    return NOT_AN_INDEX;
...
{ // only LC element can be re returned
  ...
  */
  ...
  { // starting index at centre of structure not found (probably a hole or concave mesh)
    midTet = 0;
  }
  ...
  {                                 // for each coord
    ...
    { // recursive search failed, try brute force
      ...
      {                                          // BRUTE FORCE FAIL IS ALLOWED (NODE MAY BE OUTSIDE MESH)
