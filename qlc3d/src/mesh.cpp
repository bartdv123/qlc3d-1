...
Mesh::Mesh(unsigned int dimension, unsigned int nodesPerElement) : Dimension{dimension}, nNodes{nodesPerElement}, nElements{0}, TotalSize{0} {}
...
void Mesh::setAllNodes(idx *nodes)
{
  ...
  { // assumes already correct size TODO: don't assume
...
void Mesh::setConnectedVolume(Mesh *vol)
{
  ...
  {                                                // loop over surface elements
    if ((getMaterialNumber(i) < MAT_ELECTRODE1) || // exclude eletrode only surfaces as
        (getMaterialNumber(i) > MAT_ELECTRODE9))
    { // these are fixed, and do not need to know
      ...
      else
      {
        throw std::runtime_error(format("Triangle {} is connected to {} tetrahedra, but expected 2, "
                                        "in {}, {}",
                                        i, final.size(), __FILE__, __func__));
      }
...
void Mesh::CompleteNodesSet(const idx elem, std::vector<idx> &nodes) const
{
  ...
  if (nodes.size() != (unsigned int)this->getnNodes())
  {
    RUNTIME_ERROR(format("Return size does not match the number of nodes in element. "
                         "Element {} has {} nodes, return vector has {}",
                         elem, getnNodes(), nodes.size()));
  }
#endif
}
...
void Mesh::calculateDeterminants3D(const Coordinates &coords)
{
  ...
  for (idx i = 0; i < getnElements(); i++)
  {
    ...
    if (determinants[i] < 0)
    { // should reorder nodes to ensure positive determinant
...
void Mesh::calculateSurfaceNormals(const Coordinates &coords, Mesh *tets)
{
  ...
  for (idx i = 0; i < getnElements(); i++)
  {
    ...
    if (t != NOT_AN_INDEX)
    { // index may only exist for LC tet connections.
...
void Mesh::listFixLCSurfaces(std::vector<idx> &nodes, const idx FixLCNumber) const
{
  ...
  for (idx i = 0; i < getnElements(); i++)
  {
    ...
    if (curFixLCNum == FixLCNumber)
    { // if matching FixLC number
      for (idx n = 0; n < this->getnNodes(); n++)
      {
...
void Mesh::gen_p_to_elem(vector<set<idx>> &p_to_elem) const
{
  ...
  for (idx i = 0; i < this->getnElements(); i++)
  { // for every element
    for (idx j = 0; j < this->getnNodes(); j++)
    { // for every node
...
vector<set<idx>> Mesh::gen_p_to_elem_return(vector<set<idx>> &p_to_elem) const
{
  // find largest node number
  idx maxNodeNumber = 0;
  for (const auto &n : nodes)
...
void Mesh::findNodesConnectedToNode(const Mesh &mesh, idx nodeIndex, std::set<idx> &connectedNodes, vector<set<idx>> &map) const
{

  // Retrieve the elements connected to the specified node
  const std::set<idx> &connectedElements = map[nodeIndex];

  // Extract the nodes from the connected elements
  for (const idx &elemIndex : connectedElements)
  {
    for (idx j = 0; j < mesh.getnNodes(); ++j)
    {
      connectedNodes.insert(mesh.getNode(elemIndex, j));
    }
  }
}
