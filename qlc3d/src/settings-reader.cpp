...
SettingsReader::SettingsReader(const std::filesystem::path &fileName) : fileName_(fileName),
...
void readDouble(Reader &reader, const std::string &key, std::function<void(double)> consumer)

// Add support for using expressions in double values

{
  auto value = reader.getOptional<std::string>(key);
  if (value)
  {
    ...
    try
    {
      ...
      int err;
      te_expr *expr = te_compile(strValue.c_str(), nullptr, 0, &err);
      if (!expr)
      {
        throw std::runtime_error("Failed to compile expression: " + strValue);
      }
...
void readArrayOfStrings(Reader &r, const std::string &key, std::function<void(std::vector<std::string>)> consumer)
{
  if (auto v = r.getOptional<std::vector<std::string>>(key))
  {
    consumer(v.value());
  }
}
...
void SettingsReader::readSimu(Reader &reader)
{
  ...
  readDouble(reader, SFK_DT, [&](double v)
             { builder.initialTimeStep(v); });
  // if (auto t = reader.getOptional<double>(SFK_DT)) { builder.initialTimeStep(t.value()); }
  if (auto s = reader.getOptional<string>(SFK_Q_MATRIX_SOLVER))
  {
    builder.qMatrixSolver(s.value());
  }
  if (auto v = reader.getOptional<double>(SFK_TARGET_DQ))
  {
    builder.targetDQ(v.value());
  }
  if (auto v = reader.getOptional<double>(SFK_MAX_ERROR))
  {
    builder.maxError(v.value());
  }
  if (auto v = reader.getOptional<string>(SFK_END_CRITERION))
  {
    builder.endCriterion(v.value());
  }
  if (auto v = reader.getOptional<string>(SFK_LOAD_Q))
  {
    builder.loadQ(v.value());
  }
  if (auto v = reader.getOptional<string>(SFK_SAVE_DIR))
  {
    builder.saveDir(v.value());
  }
  if (auto v = reader.getOptional<double>(SFK_END_VALUE))
  {
    builder.endValue(v.value());
  }
  if (auto v = reader.getOptional<int>(SFK_OUTPUT_FORMAT))
  {
    builder.outputFormat(v.value());
  }
  if (auto v = reader.getOptional<int>(SFK_OUTPUT_ENERGY))
  {
    builder.outputEnergy(v.value());
  }
  // if (auto v = reader.getOptional<int>(SFK_SAVE_FORMAT)) { builder.saveFormat(v.value()); } // TODO list of enum
  if (auto v = reader.getOptional<int>(SFK_SAVE_ITER))
  {
    builder.saveIter(v.value());
  }
  if (auto v = reader.getOptional<double>(SFK_SAVE_TIME))
  {
    builder.saveTime(v.value());
  }
  if (auto v = reader.getOptional<int>(SFK_NUM_ASSEMBLY_THREADS))
  {
    builder.numAssemblyThreads(v.value());
  }
  if (auto v = reader.getOptional<int>(SFK_NUM_MATRIX_SOLVER_THREADS))
  {
    builder.numMatrixSolverThreads(v.value());
  }
  ...
  if (reader.isValueArrayOfNumbers(SFK_STRETCH_VECTOR))
  {
    auto v = reader.getOptional<vector<double>>(SFK_STRETCH_VECTOR);
    // Case 1: StretchVector is a vector of doubles
    auto sv = v.value();
    if (sv.size() != 3)
    {
      throw std::runtime_error(SFK_STRETCH_VECTOR + " length should be 3.");
    }
    builder.stretchVector(sv[0], sv[1], sv[2]);
  }
  else if (reader.isValueArrayOfStrings(SFK_STRETCH_VECTOR))
  {
    auto v = reader.getOptional<vector<string>>(SFK_STRETCH_VECTOR);
    // Case 2: StretchVector is a vector of expressions
    auto sv = v.value();
    if (sv.size() != 3)
    {
      throw std::runtime_error(SFK_STRETCH_VECTOR + " length should be 3.");
    ...
    std::vector<te_expr *> compiledExpressions;
    try
    {
      double values[3];
      for (int i = 0; i < 3; ++i)
      {
        int err;
        te_expr *compiled = te_compile(sv[i].c_str(), nullptr, 0, &err);
        if (!compiled)
        {
          throw std::runtime_error("Failed to compile expression: " + sv[i]);
        }
        compiledExpressions.push_back(compiled);

        // Evaluate the expression
        values[i] = te_eval(compiled);
      }

      // Use evaluated values
      builder.stretchVector(values[0], values[1], values[2]);
    }
    catch (const std::exception &e)
    {
      // Cleanup compiled expressions on error
      for (te_expr *compiled : compiledExpressions)
      {
        te_free(compiled);
      }
  ...
  {
    throw std::runtime_error("Failed to read " + SFK_STRETCH_VECTOR + ": no valid input found.");
  }
...
{
  LCBuilder builder;
  readDouble(reader, SFK_K11, [&](double k11)
             { builder.K11(k11); });
  readDouble(reader, SFK_K22, [&](double k22)
             { builder.K22(k22); });
  readDouble(reader, SFK_K24, [&](double k24)
             { builder.K24(k24); });
  readDouble(reader, SFK_K33, [&](double k33)
             { builder.K33(k33); });
  readDouble(reader, SFK_p0, [&](double p0)
             { builder.p0(p0); });
  readDouble(reader, SFK_A, [&](double a)
             { builder.A(a); });
  readDouble(reader, SFK_B, [&](double b)
             { builder.B(b); });
  readDouble(reader, SFK_C, [&](double c)
             { builder.C(c); });
  readDouble(reader, SFK_EPS_PAR, [&](double e)
             { builder.eps_par(e); });
  readDouble(reader, SFK_EPS_PER, [&](double e)
             { builder.eps_per(e); });
  readDouble(reader, SFK_E1, [&](double e)
             { builder.e1(e); });
  readDouble(reader, SFK_E3, [&](double e)
             { builder.e3(e); });
  readDouble(reader, SFK_GAMMA1, [&](double g)
             { builder.gamma1(g); });
...
void SettingsReader::readSolverSettings(Reader &reader)
{
  ...
  readInt(reader, SFK_NUM_ASSEMBLY_THREADS, [&](int v)
          { solverSettings_->setnThreads(v); });

  readInt(reader, SFK_Q_SOLVER, [&](int v)
          { solverSettings_->setQ_Solver(v); });
  readInt(reader, SFK_Q_NEWTON_PANIC_ITER, [&](int v)
          { solverSettings_->setQ_Newton_Panic_Iter(v); });
  readDouble(reader, SFK_Q_NEWTON_PANIC_COEFF, [&](double v)
             { solverSettings_->setQ_Newton_Panic_Coeff(v); });
  readInt(reader, SFK_Q_PCG_PRECONDITIONER, [&](int v)
          { solverSettings_->setQ_PCG_Preconditioner(v); });
  readInt(reader, SFK_Q_PCG_MAXITER, [&](int v)
          { solverSettings_->setQ_PCG_Maxiter(v); });
  readDouble(reader, SFK_Q_PCG_TOLER, [&](double v)
             { solverSettings_->setQ_PCG_Toler(v); });
  readInt(reader, SFK_Q_GMRES_PRECONDITIONER, [&](int v)
          { solverSettings_->setQ_GMRES_Preconditioner(v); });
  readInt(reader, SFK_Q_GMRES_MAXITER, [&](int v)
          { solverSettings_->setQ_GMRES_Maxiter(v); });
  readInt(reader, SFK_Q_GMRES_RESTART, [&](int v)
          { solverSettings_->setQ_GMRES_Restart(v); });
  readDouble(reader, SFK_Q_GMRES_TOLER, [&](double v)
             { solverSettings_->setQ_GMRES_Toler(v); });

  readInt(reader, SFK_V_SOLVER, [&](int v)
          { solverSettings_->setV_Solver(v); });
  readInt(reader, SFK_V_PCG_PRECONDITIONER, [&](int v)
          { solverSettings_->setV_PCG_Preconditioner(v); });
  readInt(reader, SFK_V_PCG_MAXITER, [&](int v)
          { solverSettings_->setV_PCG_Maxiter(v); });
  readDouble(reader, SFK_V_PCG_TOLER, [&](double v)
             { solverSettings_->setV_PCG_Toler(v); });

  readInt(reader, SFK_V_GMRES_PRECONDITIONER, [&](int v)
          { solverSettings_->setV_GMRES_Preconditioner(v); });
  readInt(reader, SFK_V_GMRES_MAXITER, [&](int v)
          { solverSettings_->setV_GMRES_Maxiter(v); });
  readInt(reader, SFK_V_GMRES_RESTART, [&](int v)
          { solverSettings_->setV_GMRES_Restart(v); });
  readDouble(reader, SFK_V_GMRES_TOLER, [&](double v)
             { solverSettings_->setV_GMRES_Toler(v); });
}
    ...
    for (int i = 1; i <= 99; i++)
    {
      ...
      if (type == "strong")
      {
        ...
        if (reader.isValueArrayOfNumbers(key))
        {
          ...
          alignment_->addSurface(overrideVolume.has_value() ? Surface::ofStrongAnchoring(i, tiltAngleDegrees, twistAngleDegrees, overrideVolume.value()) : Surface::ofStrongAnchoring(i, tiltAngleDegrees, twistAngleDegrees));
        }
        else if (reader.isValueArrayOfStrings(key))
        {
          ...
          alignment_->addSurface(overrideVolume.has_value() ? Surface::ofStrongAnchoring(i, tiltExpression, twistExpression, overrideVolume.value()) : Surface::ofStrongAnchoring(i, tiltExpression, twistExpression));
        }
        else
        {
      ...
      else if (type == "weak")
      {
        ...
        alignment_->addSurface(overrideVolume.has_value() ? Surface::ofWeakAnchoring(i, easyAngles[0], easyAngles[1], strength, k1, k2, overrideVolume.value()) : Surface::ofWeakAnchoring(i, easyAngles[0], easyAngles[1], strength, k1, k2));
      }
      else if (type == "degenerate")
      {
        ...
        if (strength >= 0)
        {
          alignment_->addSurface(overrideVolume.has_value() ? Surface::ofPlanarDegenerate(i, strength, overrideVolume.value()) : Surface::ofPlanarDegenerate(i, strength));
        }
        else
        {
          alignment()->addSurface(overrideVolume.has_value() ? Surface::ofWeakHomeotropic(i, -strength, overrideVolume.value()) : Surface::ofWeakHomeotropic(i, -strength));
        }
      }
      else if (type == "weakhomeotropic")
      {
        auto strength = reader.getValueByKey<double>(keyBase + ".Strength");
        alignment_->addSurface(overrideVolume.has_value() ? Surface::ofWeakHomeotropic(i, strength, overrideVolume.value()) : Surface::ofWeakHomeotropic(i, strength));
      }
      else if (type == "freeze")
      {
  ...
  for (int boxNum = 1; boxNum < MAX_NUM_BOXES; boxNum++)
  {
    ...
    if (reader.containsKey(typeKey))
    {
      ...
      */
      ...
      { // assume it is an analytic expression
