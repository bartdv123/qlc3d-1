...
SolutionVector::SolutionVector() : nDoF(0),
                                   nFixed(0),
                                   nDimensions(0),
                                   nFreeNodes(0) {}

SolutionVector::SolutionVector(idx np, idx dim) : nDoF(np),
                                                  nFixed(0),
                                                  nDimensions(dim),
                                                  nFreeNodes(np)
{
...
void SolutionVector::setBooleanFixedNodeList()
{
    ...
    for (idx i = 0; i < numFixedDoFs; i++)
    { // then set only fixed nodes to true
...
void SolutionVector::setPeriodicEquNodes(const Geometry &geom)
{
    ...
    */
    ...
    {
        if (elimt.at(i) != i)
        { // IF i'th NODE IS DEPENDENT
            for (idx j = i; j < nDoF; j++)
            { // SHIFT DOWN ALL DOF INDEXES AFTER IT
                elima.at(j)--;
            }
        }
    }
    ...
    for (idx i = 0; i < nDoF; i++)
    { // SET CORRECT VALUES
        if ((elimt.at(i) != i) && (elimt.at(i) != NOT_AN_INDEX))
        {                                        // IF i'th NODE IS DEPENDENT ( AND NOT FIXED)
            elima.at(i) = elima.at(elimt.at(i)); // IT WILL DEPEND ON THE CORRECTED DOF INDEX
        }
        else if (elimt.at(i) == NOT_AN_INDEX)
        { // KEEP FIXED NODE FLAGS
